  ...
  function loadScans() {
    return doAjax2(
      ...
      (data) => {
        if (data.success) {
          dispatch(
            createAction(T.scanLog, { items: data.data.list, ajaxStatus: 2 })
          );
          props.setState({
            ...props.state,
      ...
      (textStatus) => {
        ...
        dispatch(createAction(T.scanLog, { items: null, ajaxStatus: 3 }));
      }
    );
  }
  useEffect(() => {
    if (2 === scanLog.ajaxStatus) return;
    dispatch(createAction(T.scanLog, { ajaxStatus: 1 }));
    const xhr = loadScans();
    return () => {
      if (xhr) {
  ...
  return (
    <div>
      <div style={{ textAlign: "left", marginBottom: "10px" }}>
        <button
          disabled={1 === scanLog.ajaxStatus}
          onClick={() => dispatch(createAction(T.scanLog, { ajaxStatus: 1 }))}
        >
          Refresh
        </button>
      </div>
